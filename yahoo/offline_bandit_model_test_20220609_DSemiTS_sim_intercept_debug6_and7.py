...
best_perf = 0.
for i in range(len(total_last_mean_reward)):
    model = algorithms[i]
...
# version 7
iter_discount_cumulative_reward_list = []
iter_discount_mean_reward_list = []
iter_discount_last_mean_reward = []

log_file_ind = '1'
with open('Results/iter_cumulative_reward_list_debug7_%s.pkl' % (log_file_ind), 'rb') as f:
    cumulative_reward_list = pickle.load(f)
with open('Results/iter_mean_reward_list_debug7_%s.pkl' % (log_file_ind), 'rb') as f:
    mean_reward_list = pickle.load(f)
with open('Results/iter_last_mean_reward_debug7_%s.pkl' % (log_file_ind), 'rb') as f:
    last_mean_reward = pickle.load(f)

iter_discount_cumulative_reward_list.append(cumulative_reward_list)
iter_discount_mean_reward_list.append(mean_reward_list)
iter_discount_last_mean_reward.append(last_mean_reward)
...
total_last_mean_reward = np.repeat(0.,len(cumulative_reward_list[0]))
for i in range(len(iter_discount_last_mean_reward)):
    total_last_mean_reward += np.array(iter_discount_last_mean_reward[i]).reshape(-1)
total_last_mean_reward = total_last_mean_reward/len(iter_discount_last_mean_reward)  
...
for i in range(len(total_last_mean_reward)):
    ...
    if bf_model != model.algorithm:
        if i > 0:
            best_model_gamma_dict[bf_model] = best_param
            print('%s : %.5f' % (bf_model, best_perf))
        best_param = param
        bf_model = model.algorithm
        best_perf = perf
...
best_model_gamma_dict['DiscountedSemiparamLinTS_context_full_updateFalse_disjointFalse_adjustTrue'], lbd, n_arms, context="full", update_option = False, disjoint_option = False, adjust_nu = True))

        
algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, 
best_model_param_dict['DiscountedSemiparamLinTS_context_full_updateFalse_disjointFalse_adjustFalse'], 
best_model_gamma_dict['DiscountedSemiparamLinTS_context_full_updateFalse_disjointFalse_adjustFalse'], lbd, n_arms, context="full", update_option = False, disjoint_option = False, adjust_nu = False))        


        
algorithms.append(Discounted_LinUCB_disjoint(n_user_features, n_item_features, best_model_gamma_dict['Discounted_LinUCB_context_full_updateFalse_disjointFalse'], lbd, 
...
for t, event in enumerate(events):
    ...
    if t % 10000000 == 0:
        with open('Results/Prop_sim_mean_reward_list_int_debug6_7_afTunning_para%d.pkl' % (proc_ind), 'wb') as f:
            pickle.dump(mean_reward_list, f)

        # Save MAB model
        with open('model/Prop_sim_R6A_CMABs_int_debug6_7_afTunning_para%d.pkl' % (proc_ind), 'wb') as f:
...
with open('Results/Prop_sim_mean_reward_list_int_debug6_7_afTunning_para%d.pkl' % (proc_ind), 'rb') as f:
    mean_reward_list = pickle.load(f)

with open('model/Prop_sim_R6A_CMABs_int_debug6_7_afTunning_para%d.pkl' % (proc_ind), 'rb') as f:
