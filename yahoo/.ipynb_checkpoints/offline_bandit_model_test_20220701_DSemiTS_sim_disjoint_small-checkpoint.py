...
log_file = os.path.join(save_log_dir,'offline_model_test_disjoint_prop_%s_small.txt' % (time_mark))
...
## hyper-param
hyper_param_list=[0.01, 0.012, 0.008]
#hyper_param= 0.1
...
with open('Results/Prop_disjoint_sim_mean_reward_list_%s_small.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_disjoint_sim_R6A_CMABs_%s_small.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_disjoint_sim_%s_MeanReward%s_small.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_MeanReward%s_small.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_CumulativeReward%s_small.png'%(time_mark)))
plt.show()
