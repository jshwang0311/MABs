...
log_file = os.path.join(save_log_dir,'offline_model_test_hybrid_prop_int_%s.txt' % (time_mark))
...
with open('Results/Prop_hybrid_sim_mean_reward_list_int_%s.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_hybrid_sim_R6A_CMABs_int_%s.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_hybrid_sim_%s_MeanReward%s_int.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_hybrid_sim_Best_MeanReward%s_int.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_hybrid_sim_Best_CumulativeReward%s_int.png'%(time_mark)))
plt.show()
