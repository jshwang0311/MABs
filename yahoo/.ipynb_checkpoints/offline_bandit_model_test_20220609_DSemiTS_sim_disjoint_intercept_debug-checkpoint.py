...
log_file = os.path.join(save_log_dir,'offline_model_test_disjoint_prop_int_debug_%s.txt' % (time_mark))
...
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user_interaction", update_option = False, disjoint_option = True, adjust_nu = True))
...
with open('Results/Prop_disjoint_sim_mean_reward_list_int_debug_%s.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_disjoint_sim_R6A_CMABs_int_debug_%s.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_disjoint_sim_%s_MeanReward%s_int_debug.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_MeanReward%s_int_debug.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_CumulativeReward%s_int_debug.png'%(time_mark)))
plt.show()
