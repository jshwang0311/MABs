...
log_file = os.path.join(save_log_dir,'offline_model_test_disjoint_prop_%s.txt' % (time_mark))
...
lbd = 1.
gamma_list = [0.999, 0.99, 0.95, 0.9, 0.7]
...
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user", update_option = False, disjoint_option = True))
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user_interaction", update_option = False, disjoint_option = True))

        
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        algorithms.append(Discounted_LinUCB_disjoint(n_user_features, n_item_features, gamma, lbd, hyper_param, n_arms, context="user", update_option = False, disjoint_option = True))
...
for hyper_param in hyper_param_list:
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user", False, True))
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user_interaction", False, True))
...
with open('Results/Prop_disjoint_sim_mean_reward_list_%s.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_disjoint_sim_R6A_CMABs_%s.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_disjoint_sim_%s_MeanReward%s.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_MeanReward%s.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_disjoint_sim_Best_CumulativeReward%s.png'%(time_mark)))
plt.show()
