...
change_model.coef_[sample_idx] = change_model.coef_[sample_idx] + nonstationary
...
for simul in range(simul_n):
    ...
    CO=cumul_opt(T, p_min, p_max, Data, initial_model, change_model, change_point, user_features, item_features)
    cumulated_opt.append(CO[0])
    vs=CO[1]
    for i in range(len(linTS_list)):
        mab_model = linTS_list[i]
        cumulated_reward_linTS[i].append(cumul_model(T, Data, vs, initial_model, change_model, change_point, mab_model, user_features, item_features))
        cumulated_regret_linTS[i].append(cumulated_opt[simul] - cumulated_reward_linTS[i][simul])
    
    for i in range(len(DSemiTS_list)):
        mab_model = DSemiTS_list[i]
        cumulated_reward_DSemiTS[i].append(cumul_model(T, Data, vs, initial_model, change_model, change_point, mab_model, user_features, item_features))
        cumulated_regret_DSemiTS[i].append(cumulated_opt[simul] - cumulated_reward_DSemiTS[i][simul])
    
    for i in range(len(DLinUCB_list)):
        mab_model = DLinUCB_list[i]
        cumulated_reward_DLinUCB[i].append(cumul_model(T, Data, vs, initial_model, change_model, change_point, mab_model, user_features, item_features))
        cumulated_regret_DLinUCB[i].append(cumulated_opt[simul] - cumulated_reward_DLinUCB[i][simul])
        
    for i in range(len(SemiTS_list)):
        mab_model = SemiTS_list[i]
        cumulated_reward_SemiTS[i].append(cumul_model(T, Data, vs, initial_model, change_model, change_point, mab_model, user_features, item_features))
        cumulated_regret_SemiTS[i].append(cumulated_opt[simul] - cumulated_reward_SemiTS[i][simul])
        
    
with open('Results/linTS_cum_reward_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_reward_linTS, f)
with open('Results/linTS_cum_regret_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_regret_linTS, f)

with open('Results/DSemiTS_cum_reward_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_reward_DSemiTS, f)
with open('Results/DSemiTS_cum_regret_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_regret_DSemiTS, f)
    
with open('Results/DLinUCB_cum_reward_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_reward_DLinUCB, f)
with open('Results/DLinUCB_cum_regret_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_regret_DLinUCB, f)
    
with open('Results/SemiTS_cum_reward_sim_sythetic_data_v0.pkl', 'wb') as f:
    pickle.dump(cumulated_reward_SemiTS, f)
with open('Results/SemiTS_cum_regret_sim_sythetic_data_v0.pkl', 'wb') as f:
...
print(min(Val_linTS))
print(min(Val_DSemiTS))
print(min(Val_DLinUCB))
print(min(Val_SemiTS))