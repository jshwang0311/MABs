...
log_file = os.path.join(save_log_dir,'offline_model_test_prop_int_debug1_%s.txt' % (time_mark))
...
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user_interaction", update_option = False, disjoint_option = False, adjust_nu = True))
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user_interaction", update_option = False, disjoint_option = False, adjust_nu = False))
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user", update_option = False, disjoint_option = False, adjust_nu = True))
        algorithms.append(Discounted_Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, gamma, lbd, n_arms, context="user", update_option = False, disjoint_option = False, adjust_nu = False))

        
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        algorithms.append(Discounted_LinUCB_disjoint(n_user_features, n_item_features, gamma, lbd, hyper_param, n_arms, context="user_interaction", update_option = False, disjoint_option = False))
        algorithms.append(Discounted_LinUCB_disjoint(n_user_features, n_item_features, gamma, lbd, hyper_param, n_arms, context="user", update_option = False, disjoint_option = False))
        


for hyper_param in hyper_param_list:
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user_interaction", False, False, True))
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user_interaction", False, False, False))
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user", False, False, True))
    algorithms.append(Semiparam_LinTS_disjoint(n_user_features, n_item_features, hyper_param, n_arms, "user", False, False, False))
    
    
for hyper_param in hyper_param_list:
    algorithms.append(LinearContextualThompsonSampling(n_user_features, n_item_features, hyper_param, "user_interaction", False))
...
with open('Results/Prop_sim_mean_reward_list_int_debug1_%s.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_sim_R6A_CMABs_int_debug1_%s.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_sim_%s_MeanReward%s_int_debug1.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_sim_Best_MeanReward%s_int_debug1.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_sim_Best_CumulativeReward%s_int_debug1.png'%(time_mark)))
plt.show()
