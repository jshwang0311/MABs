...
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        ...
        algorithms.append(Discounted_Semiparam_LinTS(n_user_features, n_item_features, hyper_param, gamma, lbd, context="full", update_option = False))
...
for hyper_param in hyper_param_list:
    for gamma in gamma_list:
        ...
        algorithms.append(Discounted_LinUCB(n_user_features, n_item_features, gamma, lbd, hyper_param, context="full", update_option = False))
...
for hyper_param in hyper_param_list:
    ...
    algorithms.append(Semiparam_LinTS(n_user_features, n_item_features, hyper_param, "full", False))
...
with open('Results/Prop_sim_mean_reward_list_%s.pkl' % (time_mark), 'wb') as f:
    pickle.dump(mean_reward_list, f)

# Save MAB model
with open('model/Prop_sim_R6A_CMABs_%s.pkl' % (time_mark), 'wb') as f:
...
for alg_cate_idx in range(len(alg_cate_list)):
    ...
    #plt.show()
    plt.savefig(os.path.join('Results','Prop_sim_%s_MeanReward%s.png'%(alg_cate, time_mark)))
    plt.show()
...
plt.ylabel("Mean Reward")
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_sim_Best_MeanReward%s.png'%(time_mark)))
plt.show()
...
plt.legend(loc='best', bbox_to_anchor=(1, 0.5))
plt.savefig(os.path.join('Results','Prop_sim_Best_CumulativeReward%s.png'%(time_mark)))
plt.show()