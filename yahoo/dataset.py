...
def make_data_from_yahoo_events(filenames):
    """
    ...
    global X, events, displays, rewards, articles, features, n_arms, n_events
    X = []
    events = []
    displays = []
    rewards = []
        ...
        for line in f:
            ...
            if (len(cols) - 10) % 7 != 0:
                ...
                for i in range(10, len(cols) - 6, 7):
                    ...
                    pool_ids.append(id)
                X.append(np.expand_dims(np.array([float(x[2:]) for x in cols[4:10]]),0))
                displays.append(articles.index(cols[1]))
