...
import dataset
files = ("../data/R6/ydata-fp-td-clicks-v1_0.20090501","../data/R6/ydata-fp-td-clicks-v1_0.20090502")
...
log_file = os.path.join(save_log_dir,'offline_model_test_prop_int_debug5_%s.txt' % (time_mark))
...
with open('Results/iter_cumulative_reward_list_debug5.pkl', 'wb') as f:
    pickle.dump(iter_cumulative_reward_list, f)
with open('Results/iter_mean_reward_list_debug5.pkl', 'wb') as f:
    pickle.dump(iter_mean_reward_list, f)
with open('Results/iter_last_mean_reward_debug5.pkl', 'wb') as f:
...
formatter = logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s')
time_mark = datetime.datetime.today().strftime('%Y%m%d')
log_file = os.path.join(save_log_dir,'offline_model_test_prop_int_debug5_afTunning_para%d.txt' % (proc_ind))
...
with open('Results/iter_cumulative_reward_list_debug5.pkl' , 'rb') as f:
    iter_cumulative_reward_list = pickle.load(f)
with open('Results/iter_mean_reward_list_debug5.pkl' , 'rb') as f:
    iter_mean_reward_list = pickle.load(f)
with open('Results/iter_last_mean_reward_debug5.pkl' , 'rb') as f:
...
##################################################################
log_file_ind = '3'
with open('Results/iter_cumulative_reward_list_debug5_%s.pkl' % (log_file_ind), 'rb') as f:
    cumulative_reward_list = pickle.load(f)
with open('Results/iter_mean_reward_list_debug5_%s.pkl' % (log_file_ind), 'rb') as f:
    mean_reward_list = pickle.load(f)
with open('Results/iter_last_mean_reward_debug5_%s.pkl' % (log_file_ind), 'rb') as f:
...
for t, event in enumerate(events):
    ...
    if t % 100000 == 0:
        logger.info('###### %d th round complete!' % (t))
    if t % 10000000 == 0:
        with open('Results/Prop_sim_mean_reward_list_int_debug5_afTunning_para%d.pkl' % (proc_ind), 'wb') as f:
            pickle.dump(mean_reward_list, f)

        # Save MAB model
        with open('model/Prop_sim_R6A_CMABs_int_debug5_afTunning_para%d.pkl' % (proc_ind), 'wb') as f:
...
with open('Results/Prop_sim_mean_reward_list_int_debug5_afTunning_para%d.pkl' % (proc_ind), 'rb') as f:
    mean_reward_list = pickle.load(f)

with open('model/Prop_sim_R6A_CMABs_int_debug5_afTunning_para%d.pkl' % (proc_ind), 'rb') as f:
