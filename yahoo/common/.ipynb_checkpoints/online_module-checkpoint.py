...
class Online_module:
    ...
    def learn(self, user, offered, reward, pool_idx, pool_item_features):
        for alg_idx in range(len(self.algorithms)):
            ...
            if self.algorithms[alg_idx].update_option:
                chosen = pool_idx[np.where(np.array(pool_idx) == offered)[0][0]]
                self.algorithms[alg_idx].update(offered, reward, user, pool_idx, pool_item_features[chosen])
                # save recent model
    ...
    def evaluate(self, user, offered, reward, pool_idx, pool_item_features):
        reward_idx = 0
        for alg_idx in range(len(self.algorithms)):
            # temporary implement.
            ## recent alg
